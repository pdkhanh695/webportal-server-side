...
const postCreate = async (parent, args, { req, pubsub }) => {
  ...
  let newPost = await new Post({
    ...args.input, // give us all the argument of the input
...
const allPosts = async (parent, args) => {
  ...
  return await Post.find({})
    ...
    .limit(perPage)
    .sort({ createdAt: -1 }) // to make sure sort newly created post appear on the top
    .exec();
};
const postsByUser = async (parent, args, { req }) => {
  ...
  return await Post.find({ postedBy: currentUserFromDb })
    ...
    .sort({ createdAt: -1 }); //sort by newly created one
};
